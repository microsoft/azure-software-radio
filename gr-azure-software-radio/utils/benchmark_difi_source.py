...
class difi_benchmark(gr.top_block):
    def __init__(self):
        ...
        self.difi_sink = azure_software_radio.difi_sink_cpp_fc32(0, 0, '127.0.0.1', 3333, bool(False), samples_per_packet, 0, int(352), int(samp_rate), int(0), 0, 1000, int(72), int(8), int(2), 127, 0, 1.0, -1.0)
...
class difi_benchmark(gr.top_block):
    def __init__(self):
        ...
        self.azure_software_radio_difi_source_cpp_0 = azure_software_radio.difi_source_cpp_fc32('127.0.0.1', 3334, 0, 8, 0)
        self.connect(self.azure_software_radio_difi_source_cpp_0, self.null_sink)
...
for trial in range(20): # monte carlo style
    ...
    for i in range(10): # hones in on the pass/fail sweet spot
        delay = int((lowest_working_delay - highest_failing_delay)/2) # Hone in on it
        delay += int(np.random.uniform(-100, 100)) # helps add some variations between monte carlo runs
        tb = difi_benchmark()
        ...
        samples_out = tb.azure_software_radio_difi_source_cpp_0.nitems_written(0) # max value is ~4 billion
        rate = samples_recorded*num_repeats/time_elapsed
        rates.append(rate)
        percent = samples_out/(samples_recorded*num_repeats)*100
        percents.append(percent)
        print('Recieved ' + str(percent) + '% of samples sent at a sample rate of ' + str(rate/1e6) + " MHz")
